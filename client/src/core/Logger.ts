...
class GameLogger {
  ...
  private getLogLevel(environment: string): LogLevel {
    switch (environment) {
      ...
      case 'preview':
        return LogLevel.ERROR; // TASK 8 FIX: Only show errors in preview to reduce spam
      case 'production':
  ...
  private getEnabledCategories(environment: string): Set<LogCategory> {
    switch (environment) {
      ...
      case 'preview':
        return new Set([LogCategory.CORE, LogCategory.PLAYER]); // TASK 8 FIX: Remove NETWORK category to reduce spam
      case 'production':
